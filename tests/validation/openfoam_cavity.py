...
def live_checks():
    ...
    results = {}
    try:
        for ra in (1e2, 1e4, 1e5):
            d = os.path.join(base, "ra%g" % ra)
            os.makedirs(d)
            rep, res = ofm.run_cavity(
                d, W.CavityCase(ra=ra, cells=40, iterations=1200))
        ...
        # Tolerances are set from the KNOWN error mechanism, not tuned until
        # they passed: the wall gradient is two-point across the first cell,
        # so it is first order, and the thermal boundary layer thins as Ra
        # rises — the coarse-mesh bias therefore grows with Ra. Measured on a
        # uniform 40x40: +0.13 % at 1e4, +1.9 % at 1e5. The bands below sit
        # just outside those, so a real regression trips them while the
        # expected discretisation error does not.
        for ra, tol in ((1e4, 0.03), (1e5, 0.05)):
