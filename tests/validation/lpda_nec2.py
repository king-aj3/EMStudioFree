...
Exit codes — a run that tested NOTHING must never read as a pass:
  0  every check ran and passed ('LPDA-NEC2 GATE PASSED')
  1  a check failed, OR nec2c is absent so the solve could not run at all
  2  FreeCAD itself is unavailable ('LPDA-NEC2 GATE SKIPPED') — the battery
...
def main():
    ...
    try:
        ...
        print("LPDA-NEC2 GATE SKIPPED — needs freecadcmd (FreeCAD geometry); "
    ...
    if not shutil.which("nec2c"):
        ...
        # "lpda_nec2": "nec2" in SOLVER_REQS. After that the battery reports a
        # true skip and this branch guards the by-hand path only.
        print("LPDA-NEC2 GATE FAILED: nec2c is not on PATH — this gate solves "
              "the Carrel LPDA on NEC2 and can test nothing without it")
        return 1
    import FreeCAD
    import numpy as np
