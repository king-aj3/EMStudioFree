...
`element_designer.py`; gui_smoke 41 checks / 50 commands. (Check counts throughout this file are STATIC `check(` call sites in the named gate file — the derivable basis — gated by `capability_counts`.)
...
| **Smith chart** | ✅ validated (v1.11.0) | Results ▸ **Smith** tab: the impedance locus on the unit disc with the constant-R/X grid, VSWR 2/1.5/1.2 rings, sweep start/end markers and the best-match star, plus a read-out of Z, VSWR, return loss and whether the load is inductive or capacitive. Hand-rolled on matplotlib — **no new dependency**; the three Smith packages on PyPI are all absent from FreeCAD's bundled Python, and scikit-rf pulls pandas in to draw a chart. ⚠ The chart is normalised to the PORT's reference impedance and prints it, because the centre means "matched to this port", not "50 Ω" | `smith.py`: the circle geometry checked by sampling thousands of real impedances through Γ (NOT by construction), VSWR cross-checked against `SweepResult.vswr()` and return loss against `s11_db()` to 1e-12; mutation-proven ×4, plus a `gui_smoke` check that counts the drawn artefacts |
...
| **Pattern per swept frequency + picker** | ✅ validated (v0.90.0–0.91.0), openEMS added 2026-08-22 | NEC2 multi-frequency `FR`+`RP`; **openEMS from one broadband NF2FF recording — no extra solve at all**; **Pattern Frequencies…** dialog with editable band + a recommended step landing on S11 sample points; both pattern tabs and the 3-D export share one selection | `pattern_sweep.py`: 89 checks — N patterns from ONE run (201 in 7.18 s), per-frequency gains pinned, band round-trip, the far-field sort proven on a DESCENDING file, and the flat-band pattern-frequency guard now shared by BOTH backends (NEC2 had a bare argmin until 2026-08-24; one constant in `emstudio.post.sparams`); 11/11 + 5/5 + 3/3 + 2/2 mutations caught |
