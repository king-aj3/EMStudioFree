...
Full manual with step-by-step tutorials: [docs/USER_MANUAL.md](docs/USER_MANUAL.md)
...
**Templates**, **Tools**, **Setup**, **Help**. Every command has its own icon.
...
| Command | What it does |
|---|---|
| **Detect / Install Solvers** | Setup wizard: backend status, one-line install command (apt on Linux; native-installer/WSL2 guidance on Windows), guided no-sudo source builds with live output |
...
| **Legal notice & disclaimer** | Intended use (educational / hobbyist / experimental), no warranty and no liability, your duty to verify every result, the no-safety-critical exclusion, and the EMStudio / AJJ³ brand terms. Opens DISCLAIMER.md / TRADEMARK.md / LICENSE |
