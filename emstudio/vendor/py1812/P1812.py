...
# EMStudio vendoring change (2026-07-09): the ITU digital maps are NOT
# bundled (licensing) — load lazily, only when a call actually needs them
# (i.e. when DN/N0 are not supplied by the caller).
_DIGITAL_MAPS = None


def _digital_maps():
    global _DIGITAL_MAPS
    if _DIGITAL_MAPS is None:
        try:
            path = files("emstudio.vendor.py1812").joinpath("P1812.npz")
            with np.load(path) as npz:
                _DIGITAL_MAPS = {k: npz[k].copy() for k in npz.files}
        except FileNotFoundError:
            raise FileNotFoundError(
                "The ITU-R P.1812 digital maps (P1812.npz) are not installed "
                "(they cannot be redistributed with EMStudio). Either pass "
                "DN= and N0= explicitly, or generate P1812.npz from the "
                "official ITU zip with initiate_digital_maps.py in this "
                "directory.")
    return _DIGITAL_MAPS


class _LazyMaps:
    def __getitem__(self, k):
        return _digital_maps()[k]


DigitalMaps = _LazyMaps()
