...
import numpy as np
# EMStudio vendoring change (2026-07-09): matplotlib import made lazy
# (only the optional plotTca/plotTeff1 debug plots need it).
...
def bt_loss(f, t, heff, h2, R2, area, d_v, path_c, pathinfo, *args):
    ...
    # EMStudio local patch (2026-07-26): np.mat was removed in NumPy 2.0;
    # np.asmatrix is the documented drop-in alias (see PROVENANCE.md).
    figure_rec = np.asmatrix(figure_rec_array)
...
def step7_normal(figureStep6, h1, dinf, dsup, d, generalPath, f, Emaxvalue, t):
    ...
    for j in range(st, 2):
        ...
        # EMStudio local patch (2026-07-26): np.mat -> np.asmatrix (NumPy 2.0)
        tabulatedValues = np.asmatrix(exceltables[figureStep7[0,3]-1])
